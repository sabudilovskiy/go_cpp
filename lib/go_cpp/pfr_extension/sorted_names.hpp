...
consteval auto get_sorted_names() {
    ...
    auto names = pfr::names_as_array<without_ref>();
...
consteval auto get_sorted_names_with_indexes() {
    using without_ref = std::remove_cvref_t<T>;
    constexpr auto names = pfr::names_as_array<without_ref>();
    std::array<std::pair<std::string_view, std::size_t>, pfr::tuple_size_v<T>>
        names_pair{};
